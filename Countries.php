<?php
	...
	class Countries extends Loader{


		public function init($ms='global',$v=1,$url=false){
			...
			foreach (glob(__DIR__.'/dist/*') as $c) {
				$f = explode('.',$c);
				$raw = str_replace(__DIR__,$url.'loader/modules/Countries',$c);
				if( $f[count($f)-1] == 'css' ){
			...
			echo "<script async defer>var countries_config={ ms : '".strtolower($ms)."', version : ".$v.", assets : '".json_encode($assets)."'};".trim(preg_replace('/\s+/', ' ',file_get_contents(__DIR__.'/loader.js')))."</script>\n";
